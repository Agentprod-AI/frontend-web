...
export const zohoLogin = (userid: string) => {
  window.location.href = `${process.env.NEXT_PUBLIC_SERVER_URL}v2/zoho/login/${userid}`;
};
export const pipedriveLogin = (userid: string) => {
  window.location.href = `${process.env.NEXT_PUBLIC_SERVER_URL}v2/pipedrive/login/${userid}`;
};
