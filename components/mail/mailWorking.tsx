...
export function Mail({
  ...
  const fetchConversations = React.useCallback(
    async (
      ...
      try {
        let url = `v2/mailbox/${user?.id}?limit=${ITEMS_PER_PAGE}&offset=${
          (pageNum - 1) * ITEMS_PER_PAGE
        }`;

        if (campaignId) {
          url = `v2/mailbox/campaign/${campaignId}/${user?.id}?limit=${ITEMS_PER_PAGE}&offset=${
            (pageNum - 1) * ITEMS_PER_PAGE
          }`;
        }
  ...
  const loadMore = React.useCallback(() => {
    if (!loading && hasMore) {
      ...
      if (page === 1) {
        // Enable scrolling after the first load
  ...
  const handleFilterChange = React.useCallback(
    ...
    [campaign, searchTerm, fetchConversations]
  );
  ...
  const handleSearchChange = React.useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setPage(1);
      fetchConversations(campaign?.campaignId, 1, newSearchTerm, filter);
    },
    [campaign, filter, fetchConversations]
  );
