...
function PreviewContent({ subject, body }: any) {
  ...
  const [oldSubject, setOldSubject] = React.useState("");
  const [oldBody, setOldBody] = React.useState("");

  React.useEffect(() => {
    // Retrieve the values from localStorage
    const storedSubject = localStorage.getItem("autoGeneratedSubject");
    const storedBody = localStorage.getItem("autoGeneratedBody");

    if (storedSubject) setOldSubject(storedSubject);
    if (storedBody) setOldBody(storedBody);
  }, []);
          ...
          <div className="flex-col w-full">
            ...
            <div className="flex mt-2 items-center">
              ...
              <Input
                ...
                value={newPreviews ? newSubject : subject || oldSubject}
                readOnly
              />
            </div>

            <div className="flex flex-row gap-2 mt-3">
              <TextField text={newPreviews ? newBody : body || oldBody} />
            </div>
          </div>
        </div>
      </ResizablePanel>
