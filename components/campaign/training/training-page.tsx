...
export default function Training() {
  ...
  const handleLetAiWrite = async () => {
    ...
    try {
      ...
      if (response) {
        ...
        setAutoGeneratedBody(response.body); // Fixed this line
        setAutoGeneratedSubject(response.subject); // Fixed this line
        setContact(response.contact);
        setLinkedinInformation(response.linkedin_information);

        const followone = await getFollowUpOne();
  ...
  `;
          ...
          setAutoGeneratedFollowUp(autoFollowUp);
        }

        const followtwo = await getFollowUpTwo();

        if (followtwo) {
          const autoFollowUpTwo = `
  ${followtwo.subject}
    
  ${followtwo.body}
  `;
          console.log(autoFollowUpTwo);
          setAutoGeneratedFollowUpTwo(autoFollowUpTwo);
        ...
        throw new Error("Invalid response from getAutogenerateTrainingEmail");
      }
    } catch (error) {
      console.error("Failed to fetch training data:", error);
    }
